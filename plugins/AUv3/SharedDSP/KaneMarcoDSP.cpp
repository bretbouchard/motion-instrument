...
//  SharedDSP
//
//  C++ wrapper implementation for Motion Marco synthesizer DSP
//

#include "MotionDSP.h"
#include "plugins/dsp/include/dsp/MotionPureDSP.h"
#include <cstring>
#include <cmath>
#include <array>
...
class MotionDSP::Impl {
public:
    DSP::MotionPureDSP dsp;
...
MotionDSP::MotionDSP() : impl(std::make_unique<Impl>()) {}

MotionDSP::~MotionDSP() = default;

void MotionDSP::initialize(double sampleRate, int maximumFramesToRender) {
...
void MotionDSP::process(AUAudioFrameCount frameCount,
...
float MotionDSP::getParameter(AUParameterAddress address) const {
    if (address >= PARAM_COUNT) return 0.0f;
    return impl->parameterCache[address];
}

void MotionDSP::handleMIDIEvent(const uint8_t *message, uint8_t messageSize) {
...
void MotionDSP::setState(const char *stateData) {
    impl->dsp.loadPreset(stateData);
}

const char *MotionDSP::getState() const {
...
int MotionDSP::getFactoryPresetCount() const {
    return sizeof(factoryPresetNames) / sizeof(factoryPresetNames[0]);
}

const char *MotionDSP::getFactoryPresetName(int index) const {
...
void MotionDSP::loadFactoryPreset(int index) {
    // Load from presets/Motion folder
