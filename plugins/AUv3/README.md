# Motion Marco AUv3 iOS Plugin

## Project Overview

Complete iOS AUv3 instrument plugin implementation of the Motion Marco Hybrid Virtual Analog Synthesizer.

**Motion Marco** is a professional-grade 16-voice polyphonic synthesizer featuring:
...
```
ios-auv3/
├── MotionPluginApp/            # AUv3 host container app
│   ├── AppDelegate.swift
...
│   ├── MotionDSP.h            # DSP wrapper header (135 parameters)
│   ├── MotionDSP.cpp          # DSP wrapper implementation
...
```bash
# Open Xcode project
open MotionPlugin.xcodeproj

# Or build from command line
xcodebuild -project MotionPlugin.xcodeproj \
           -scheme MotionPlugin \
...
The Motion Marco features a **16-slot modulation matrix** with:
...
**Issue**: `MotionPureDSP.h not found`
```bash
# Fix: Ensure correct include path in CMakeLists.txt
# The DSP files are at: juce_backend/instruments/motion/plugins/dsp/
```

**Issue**: Linker errors for undefined symbols
```bash
# Fix: Add MotionDSP static library to linker flags
...
1. Add parameter to `MotionDSP.h` enum
2. Register parameter in `AudioUnit.swift` parameterDefinitions
3. Map parameter in `MotionDSP.cpp` setParameter()
...
Place in `presets/Motion/` folder and add to factory preset list.
...
- [Motion Marco Desktop Version](../../../../../../README.md)

---

**Status**: ✅ Complete - Ready for Xcode project creation and testing
**Created**: 2025-01-17
**Location**: `juce_backend/instruments/motion/ios-auv3/`
