...
**Project:** Motion Marco Instrument Family
...
typedef void* MotionAetherStringHandle;

MotionAetherStringHandle motion_aether_string_create();
void motion_aether_string_destroy(MotionAetherStringHandle handle);

void motion_aether_string_prepare(MotionAetherStringHandle handle,
                                      double sampleRate,
                                      int samplesPerBlock);
...
void motion_aether_string_process(MotionAetherStringHandle handle,
                                      float** channels,
                                      int numChannels,
                                      int numSamples,
...
void motion_aether_string_set_articulation(MotionAetherStringHandle handle,
                                               int articulationType);  // 0-5

int motion_aether_string_get_articulation(MotionAetherStringHandle handle);

void motion_aether_string_set_crossfade_time(MotionAetherStringHandle handle,
                                                 double timeMs);
...
void motion_aether_string_set_string_frequency(MotionAetherStringHandle handle,
                                                   int stringIndex,  // 0-5
                                                   float frequency);

float motion_aether_string_get_string_frequency(MotionAetherStringHandle handle,
                                                    int stringIndex);

void motion_aether_string_set_string_damping(MotionAetherStringHandle handle,
                                                 int stringIndex,
                                                 float damping);  // 0-1
...
void motion_aether_string_set_bridge_coupling(MotionAetherStringHandle handle,
                                                  float coupling);  // 0-1

float motion_aether_string_get_bridge_coupling(MotionAetherStringHandle handle);

//==============================================================================
// Body Resonator
//==============================================================================

void motion_aether_string_set_body_resonance(MotionAetherStringHandle handle,
                                                 float resonance);  // 0-1

void motion_aether_string_load_body_preset(MotionAetherStringHandle handle,
...
void motion_aether_string_enable_pedal(MotionAetherStringHandle handle,
                                          int pedalIndex,  // 0-7
                                          bool enable);

bool motion_aether_string_is_pedal_enabled(MotionAetherStringHandle handle,
                                               int pedalIndex);

void motion_aether_string_set_pedal_parameter(MotionAetherStringHandle handle,
                                                  int pedalIndex,
                                                  const char* parameterId,
                                                  float value);

float motion_aether_string_get_pedal_parameter(MotionAetherStringHandle handle,
                                                   int pedalIndex,
                                                   const char* parameterId);

void motion_aether_string_set_pedal_order(MotionAetherStringHandle handle,
...
char* motion_aether_string_save_preset(MotionAetherStringHandle handle);
bool motion_aether_string_load_preset(MotionAetherStringHandle handle,
                                          const char* jsonData);

bool motion_aether_string_validate_preset(MotionAetherStringHandle handle,
                                              const char* jsonData);

int motion_aether_string_get_num_factory_presets(MotionAetherStringHandle handle);
const char* motion_aether_string_get_factory_preset_name(MotionAetherStringHandle handle,
                                                              int index);
bool motion_aether_string_load_factory_preset(MotionAetherStringHandle handle,
                                                  int index);

void motion_aether_string_free_string(char* str);
...
class MotionAetherStringBridge {
    private var instance: OpaquePointer?

    init(sampleRate: Double = 48000.0, bufferSize: Int = 512) {
        instance = motion_aether_string_create()
        motion_aether_string_prepare(instance, sampleRate, Int32(bufferSize))

        // Load guitar body preset
        motion_aether_string_load_body_preset(instance, 0)
    }

    deinit {
        if let inst = instance {
            motion_aether_string_destroy(inst)
        }
    }

    // Articulation control
    func setArticulation(_ type: ArticulationType) {
        motion_aether_string_set_articulation(instance, type.rawValue)
    }

    func setCrossfadeTime(_ timeMs: Double) {
        motion_aether_string_set_crossfade_time(instance, timeMs)
    }

    // String control
    func setStringFrequency(_ index: Int, frequency: Float) {
        motion_aether_string_set_string_frequency(instance, Int32(index), frequency)
    }

    // Bridge coupling
    func setBridgeCoupling(_ coupling: Float) {
        motion_aether_string_set_bridge_coupling(instance, coupling)
    }

    // Body resonator
    func setBodyResonance(_ resonance: Float) {
        motion_aether_string_set_body_resonance(instance, resonance)
    }

    // Pedalboard
    func enablePedal(_ index: Int, enabled: Bool) {
        motion_aether_string_enable_pedal(instance, Int32(index), enabled)
    }

    func setPedalParameter(_ pedalIndex: Int, parameterId: String, value: Float) {
        parameterId.withCString { cString in
            motion_aether_string_set_pedal_parameter(instance, Int32(pedalIndex), cString, value)
        }
    }

    func setPedalOrder(_ order: [Int]) {
        order.withUnsafeBufferPointer { buffer in
            motion_aether_string_set_pedal_order(instance, buffer.baseAddress, Int32(order.count))
        }
    }
    ...
    func processAudio(_ output: UnsafeMutablePointer<Float>,
        ...
        if let midi = midi {
            ...
            motion_aether_string_process(inst, &channels, 2, Int32(frameCount), nil, 0)
            return true
        }
    }
    ...
    func savePreset() -> String? {
        guard let cString = motion_aether_string_save_preset(instance) else { return nil }
        defer { motion_aether_string_free_string(cString) }
        return String(cString: cString)
    }

    func loadPreset(_ jsonData: String) -> Bool {
        return jsonData.withCString { cString in
            motion_aether_string_load_preset(instance, cString)
        }
    }
}
...
**File 21: `include/ffi/MotionAetherStringFFI.h`**
...
**File 22: `src/ffi/MotionAetherStringFFI.cpp`**
...
**File 23: `tests/dsp/MotionAetherStringTests.cpp`**
...
**File 24: `presets/MotionAetherString/`**
...
**Priority:** HIGH (Critical path for Motion Marco Instrument Family)
