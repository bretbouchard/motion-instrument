...
```cpp
class MotionAetherTests : public juce::UnitTest
{
public:
...
#### `include/ffi/motion_aether_ffi.h` (~300 lines)

**C Interface:**
```c
extern "C" {
    typedef void* MotionAetherHandle;

    // Lifecycle
    MotionAetherHandle motion_aether_create();
    void motion_aether_destroy(MotionAetherHandle handle);
    void motion_aether_initialize(MotionAetherHandle handle, double sampleRate, int samplesPerBlock);

    // Audio processing
    void motion_aether_process(MotionAetherHandle handle, float** channels, int numChannels, int numSamples, const uint8_t* midiData, int midiDataSize);

    // Parameters
    int motion_aether_get_num_parameters(MotionAetherHandle handle);
    const char* motion_aether_get_parameter_id(MotionAetherHandle handle, int index);
    void motion_aether_set_parameter(MotionAetherHandle handle, const char* paramId, float value);
    float motion_aether_get_parameter(MotionAetherHandle handle, const char* paramId);

    // Presets
    void motion_aether_load_preset(MotionAetherHandle handle, const char* jsonData);
    char* motion_aether_save_preset(MotionAetherHandle handle);
    int motion_aether_get_num_presets(MotionAetherHandle handle);
    const char* motion_aether_get_preset_name(MotionAetherHandle handle, int index);
    void motion_aether_load_factory_preset(MotionAetherHandle handle, int index);
    bool motion_aether_validate_preset(MotionAetherHandle handle, const char* jsonData);

    // String memory management
    void motion_aether_free_string(char* str);
}
```
...
#### `src/ffi/motion_aether_ffi.cpp` (~700 lines)
...
#### `tests/dsp/MotionAetherTests.cpp` (~1,100 lines, 55 tests)
...
#### `presets/MotionAether/` (20 factory presets)
...
```cmake
# Aether DSP
set(MOTION_MARCO_AETHER_SOURCES
    src/dsp/AetherDSP.cpp
    src/ffi/motion_aether_ffi.cpp
)

set(MOTION_MARCO_AETHER_HEADERS
    include/dsp/AetherDSP.h
    include/ffi/motion_aether_ffi.h
)

set(MOTION_MARCO_AETHER_TESTS
    tests/dsp/MotionAetherTests.cpp
)

add_library(AetherDSP STATIC ${MOTION_MARCO_AETHER_SOURCES})
...
# FFI Bridge
add_library(MotionAetherFFI STATIC ${MOTION_MARCO_AETHER_SOURCES} src/ffi/motion_aether_ffi.cpp)
target_link_libraries(MotionAetherFFI PUBLIC AetherDSP)

# Tests
add_executable(MotionAetherTests ${MOTION_MARCO_AETHER_TESTS})
target_link_libraries(MotionAetherTests PRIVATE AetherDSP JUCE::juce_recommended_config_files JUCE::juce_recommended_lto_flags)
```
...
**Project:** Motion Marco Instrument Family - Phase 1
