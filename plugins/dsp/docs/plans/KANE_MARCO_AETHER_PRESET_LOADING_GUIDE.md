...
class MotionAetherPresetLoader {
public:
    ...
    static std::vector<std::string> getAvailablePresets(
        const std::string& presetsDir = "presets/MotionAether") {
...
/*
  ...
  // Load preset
  MotionAetherPresetLoader::PresetParameters params;
  bool success = MotionAetherPresetLoader::loadPreset(
      "presets/MotionAether/01_Ethereal_Atmosphere.json",
      aether,
      params
  );
  ...
  // List all presets
  auto presets = MotionAetherPresetLoader::getAvailablePresets();
  for (const auto& presetPath : presets) {
      std::string name, category, description;
      if (MotionAetherPresetLoader::getPresetMetadata(
