/*
  ==============================================================================
    ...
    C bridge implementation for MotionDSP - FFI layer for Swift/tvOS

    This file implements the C API wrapper around the C++ MotionDSP class.
...
#include "../include/dsp/MotionDSP.h"
#include <string>
#include <cstring>
#include <memory>
 ...
 */
struct MotionDSPInstance
{
    std::unique_ptr<MotionDSP> synth;
    std::string lastError;

    MotionDSPInstance() : synth(std::make_unique<MotionDSP>()) {}
};
                               ...
                               MotionModulationCurve curve)
{
    ...
    try
    {
        ...
        // The Motion Marco DSP has internal modulation matrix management
...
void motion_clear_all_modulation(MotionDSPInstance* instance)
{
    ...
    try
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            motion_clear_modulation(instance, i);
        }
    }
    catch (const std::exception& e)
    {
...
int motion_get_modulation_slot_count(MotionDSPInstance* instance)
{
    ...
    // Motion Marco has 16 modulation slots
    return 16;
}
...
int motion_get_latency(MotionDSPInstance* instance)
{
    ...
    try
    {
        // Motion Marco has no internal latency (zero-delay filter)
        return 0;
    }
    catch (const std::exception& e)
    {
