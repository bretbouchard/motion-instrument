/*
  ==============================================================================
    ...
    Example/test program demonstrating Motion Marco FFI usage

    This program shows how to use the Motion Marco FFI bridge from C/C++.

  ==============================================================================
*/

#include "../include/ffi/MotionFFI.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <cmath>
...
void test_lifecycle()
{
    ...
    // Create instance
    printf("Creating Motion Marco instance...\n");
    MotionDSPInstance* instance = motion_create();
    if (instance == NULL)
    {
    ...
    bool success = motion_initialize(instance, 48000.0, 512);
    if (!success)
    {
        printf("✗ FAILED: Could not initialize (error: %s)\n",
               motion_get_last_error(instance));
        motion_destroy(instance);
        return;
    }
...
void test_parameters()
{
    ...
    MotionDSPInstance* instance = motion_create();
    motion_initialize(instance, 48000.0, 512);

    // Get parameter count
    int paramCount = motion_get_parameter_count(instance);
    ...
    for (int i = 0; i < (paramCount < 5 ? paramCount : 5); ++i)
    {
        ...
        if (motion_get_parameter_id(instance, i, idBuffer, sizeof(idBuffer)))
        {
            if (motion_get_parameter_name(instance, idBuffer, nameBuffer, sizeof(nameBuffer)))
            {
                float value = motion_get_parameter_value(instance, idBuffer);
    ...
    if (motion_set_parameter_value(instance, "master_gain", 0.8f))
    {
        float newValue = motion_get_parameter_value(instance, "master_gain");
        printf("✓ New value: %.3f\n", newValue);
    }
    else
    {
...
{
    print_separator();
    printf("TEST: Macro Controls (Motion Marco Specific)\n");
    print_separator();

    MotionDSPInstance* instance = motion_create();
    motion_initialize(instance, 48000.0, 512);

    int macroCount = motion_get_macro_count(instance);
    ...
    for (int i = 0; i < macroCount; ++i)
    {
        ...
        // Set macro
        if (motion_set_macro(instance, i, testValue))
        {
            printf("  Set to %.3f\n", testValue);

            // Get macro
            float retrievedValue = motion_get_macro(instance, i);
            printf("  Retrieved: %.3f\n", retrievedValue);
...
{
    print_separator();
    printf("TEST: Modulation Matrix (Motion Marco Specific)\n");
    print_separator();

    MotionDSPInstance* instance = motion_create();
    motion_initialize(instance, 48000.0, 512);

    int slotCount = motion_get_modulation_slot_count(instance);
    printf("Modulation slot count: %d\n", slotCount);

    // Test setting a modulation routing
    printf("\nSetting modulation slot 0: LFO1 → filter_cutoff, amount=0.5\n");
    bool success = motion_set_modulation(instance,
                                             0,  // slot
                                             MOTION_MOD_SOURCE_LFO1,  // source
                                             "filter_cutoff",  // destination
                                             0.5f,  // amount
                                             MOTION_MOD_CURVE_LINEAR);  // curve
    ...
    if (motion_clear_modulation(instance, 0))
    {
...
void test_factory_presets()
{
    ...
    int presetCount = motion_get_factory_preset_count(instance);
    ...
    {
        char nameBuffer[128];
        if (motion_get_factory_preset_name(instance, i, nameBuffer, sizeof(nameBuffer)))
        {
            printf("  %2d: %s\n", i, nameBuffer);
        }
    }
    ...
    {
        printf("\nLoading preset 0...\n");
        if (motion_load_factory_preset(instance, 0))
        {
...
void test_preset_save_load()
{
    ...
    motion_set_parameter_value(instance, "master_gain", 0.7f);
    motion_set_parameter_value(instance, "osc1_waveform", 2.0f);
    motion_set_macro(instance, 0, 0.75f);

    // Save preset
    printf("\nSaving preset to JSON...\n");
    char jsonBuffer[8192];
    int jsonSize = motion_save_preset(instance, jsonBuffer, sizeof(jsonBuffer));
    ...
    if (jsonSize > 0)
    {
        ...
        if (motion_validate_preset(instance, jsonBuffer))
        {
        ...
        if (motion_get_preset_info(instance, jsonBuffer,
        ...
        motion_set_parameter_value(instance, "master_gain", 0.3f);
        printf("  master_gain changed to: %.3f\n",
               motion_get_parameter_value(instance, "master_gain"));

        printf("\nReloading preset from JSON...\n");
        if (motion_load_preset(instance, jsonBuffer))
        {
            printf("✓ Preset loaded\n");
            float restoredValue = motion_get_parameter_value(instance, "master_gain");
...
void test_audio_processing()
{
    ...
    // Load a preset
    if (motion_get_factory_preset_count(instance) > 0)
    {
        motion_load_factory_preset(instance, 0);
    }
    ...
    motion_process(instance, audioBuffer, 512, midiNoteOn, sizeof(midiNoteOn));
    ...
    // Check voice count
    int voiceCount = motion_get_active_voice_count(instance);
    printf("Active voices: %d\n", voiceCount);

    // Check latency
    int latency = motion_get_latency(instance);
...
void test_reset()
{
    ...
    motion_set_parameter_value(instance, "master_gain", 0.2f);
    motion_set_macro(instance, 0, 0.9f);
    printf("  master_gain: %.3f\n", motion_get_parameter_value(instance, "master_gain"));
    printf("  macro0: %.3f\n", motion_get_macro(instance, 0));

    // Reset
    printf("\nResetting synth...\n");
    motion_reset(instance);
...
int main(int argc, char* argv[])
{
    printf("\n");
    printf("╔════════════════════════════════════════════════════════════════════════════╗\n");
    printf("║                  Motion Marco FFI Bridge Test Program                       ║\n");
